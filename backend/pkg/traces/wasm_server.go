...
type WasmHandleTraceFunc func(trace *spec.SCNTelemetry) error

type WasmHTTPTracesServer struct {
	traceHandleFunc HandleTraceFunc
	server          *http.Server
}

func CreateWasmHTTPTracesServer(port int, traceHandleFunc HandleTraceFunc) *WasmHTTPTracesServer {
	s := &WasmHTTPTracesServer{
...
func (s *WasmHTTPTracesServer) Stop() {
